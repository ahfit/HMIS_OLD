<%@ Page Language="VB" AutoEventWireup="false" MasterPageFile="~/hacims_masterpage_admin.master" CodeFile="Search_Employee_ForLeaveCancelByHR.aspx.vb"
     Inherits="LM_Search_Employee_ForLeaveCancelByHR" EnableEventValidation="false" Theme="theme_hacims" ViewStateEncryptionMode="Never" MaintainScrollPositionOnPostback="true" %>
            ...
            <tr>
                ...
                <td colspan="2">
                    ...
                    <asp:SqlDataSource ID="SDS_Employee" runat="server"
                        ...
                        SelectCommand=" SELECT 0 AS EmpID, '---ALL---' AS EMPLOYEE_NAME UNION
 SELECT Employee.EmpID,  ISNULL(EFName, '') + ' ' + ISNULL(EMName, '') + ' ' + ISNULL(ELName, '')+' '+Cast(Employee.EmpID as varchar)+' '+' (' + Designation.Designation_Name +')' AS EMPLOYEE_NAME
FROM         Employee
 inner join Designation on Employee.DesignationID=Designation.Designation_ID
 where (Employee.ActiveStatus = 1) and (Employee.SubDeptId = @SubDeptId)" 
...
<asp:BoundField DataField="EmpID" HeaderText="Emp ID" SortExpression="EmpID" />
            ...
            <asp:HyperLinkField DataNavigateUrlFields="Emp_Leave_Id,EMPID,SubDeptId" DataNavigateUrlFormatString="~/Leave_Management/Employee_Leaves_CancelByHR.aspx?Emp_Leave_Id={0}&Emp_ID={1}&SubDeptId={2}"
    ...
    <asp:SqlDataSource ID="SqlDataSourceDepartment" runat="server" ConnectionString="<%$ ConnectionStrings:Leave_ManagementConnectionString %>"
        ProviderName="<%$ ConnectionStrings:Basic_Data_ConnectionString.ProviderName %>"
        SelectCommand="select SubDept_Id, SubDept_Name from SubDepartment">
