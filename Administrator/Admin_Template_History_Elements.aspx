<%@ page language="VB" autoeventwireup="false" masterpagefile="~/hacims_masterpage_admin.master" inherits="Administrator_Admin_Template_History_Elements, App_Web_lwhdjlgh" enableEventValidation="false" theme="theme_hacims" viewStateEncryptionMode="Never" maintainScrollPositionOnPostBack="true" %>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl_form"> 
  <tr>
    <td align="right" width="40%">Template :  </td>
    <td width="60%"><asp:DropDownList ID="DropDownListTemplate" runat="server" datasourceid="SqlDataSourceTemplates" DataTextField="Template_Name" DataValueField="Template_Id" Width="223px" AutoPostBack="True"> </asp:DropDownList>
                                <asp:SqlDataSource ID="SqlDataSourceTemplates" runat="server"
                                        ConnectionString="<%$ ConnectionStrings:Treatment_AdminConnectionString %>" SelectCommand="SELECT [Template_Id], [Template_Name] FROM [Admin_Template]  where active=1 ORDER BY [Template_Name]  " ProviderName="<%$ ConnectionStrings:Treatment_AdminConnectionString.ProviderName %>"> </asp:SqlDataSource></td>
  </tr>
  <tr>
    <td align="right">Main Group : </td>
    <td> <asp:DropDownList ID="DropDownListGroup" runat="server" datasourceid="SqlDataSource1" DataTextField="MG_Name" DataValueField="MG_Code"  appenddatabounditems="True" AutoPostBack="True" Width="223px"> </asp:DropDownList>
                              <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:Treatment_AdminConnectionString %>"
                                      SelectCommand="SELECT [MG_Code], [MG_Name] FROM [admin_History_MainGroup] ORDER BY [MG_Name]" ProviderName="<%$ ConnectionStrings:Treatment_AdminConnectionString.ProviderName %>"></asp:SqlDataSource></td>
  </tr>
  <tr>
    <td></td>
    <td><asp:Label CssClass="err"  ID="LabelEmpName" runat="server" ></asp:Label> <asp:Label CssClass="err"  ID="Label1" runat="server" ></asp:Label></td>
  </tr>
</table></div>
...
<asp:GridView ID="GridViewMGName" runat="server" AutoGenerateColumns="False"
                        DataSourceID="SqlDataSourceForGrid" Width="100%" CssClass="Grid_1" DataKeyNames="MG_Code,e_Code">
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="Add to Template">
                                    <ItemTemplate>
                                        ...
                                        <asp:HiddenField ID="HiddenFieldroweid" runat="server" Value='<%# Eval("e_Code", "{0}") %>' />
                                        <br />
                                        <asp:HiddenField ID="HiddenField_MainGroupid" runat="server" Value='<%# Eval("MG_Code", "{0}") %>' />
                                    </ItemTemplate>
                                </asp:TemplateField>
...
<br />
                            ...
                            <asp:SqlDataSource ID="SqlDataSourceDelete" runat="server" ConnectionString="<%$ ConnectionStrings:Treatment_AdminConnectionString %>"
                                DeleteCommand="DELETE FROM Admin_Template_History_Elements WHERE (Template_Id = @Template_Id) AND (Element_Id = @Element_Id)"
                ...
                ConnectionString="<%$ ConnectionStrings:Treatment_AdminConnectionString %>" DeleteCommand="DELETE FROM Admin_Template_History_Elements WHERE (Template_Id = @Template_Id) AND (Element_Id = @Element_Id)"
                InsertCommand="admin_insert_template_history_elements"
...
<asp:SqlDataSource ID="SqlDataSourceForGrid" runat="server" ConflictDetection="CompareAllValues"
                ...
                OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT     Admin_History_Elememt_Assign_To_Group.MG_Code, admin_History_MainGroup.MG_Name, Admin_History_Elememt_Assign_To_Group.e_Code, &#13;&#10;                      admin_History_Elements.e_Name&#13;&#10;FROM         admin_History_MainGroup INNER JOIN&#13;&#10;                      Admin_History_Elememt_Assign_To_Group ON &#13;&#10;                      admin_History_MainGroup.MG_Code = Admin_History_Elememt_Assign_To_Group.MG_Code INNER JOIN&#13;&#10;                      admin_History_Elements ON Admin_History_Elememt_Assign_To_Group.e_Code = admin_History_Elements.e_Code&#13;&#10; WHERE (Admin_History_Elememt_Assign_To_Group.MG_Code = @MG_Code)"
