...
Partial Class PatientSearch
    ...
    Function GetPatients() As DataTable
        ...
        Dim dtPatients As DataTable = New DataTable()
        Dim objManager As DbManager = New DbManager()

        dtPatients = objManager.ExecuteDataTable("Search_Patient_For_Receptionist", "Reg_ConnectionString", sqlParams)
        Return dtPatients
    End Function
    ...
    Protected Sub GridViewSearch_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridViewSearch.PreRender
        Try
            ...
            If GridViewSearch.Rows.Count <> 0 Then
                GridViewSearch.HeaderRow.Cells(6).Text = "Visit No."
                GridViewSearch.HeaderRow.Cells(8).Text = "Reg No."
                GridViewSearch.HeaderRow.Cells(9).Text = "Patient Name"
                'GridViewSearch.HeaderRow.Cells(22).Visible = False
                ...
                For i As Integer = 0 To GridViewSearch.Rows.Count - 1
                    If Page.IsPostBack = True Then
                        Dim prefix As String = ""
                    Else
                        prefix = GridViewSearch.Rows(i).Cells(8).Text
                        GridViewSearch.Rows(i).Cells(8).Text = GridViewSearch.Rows(i).Cells(8).Text 'prefix & " " &
                    End If
                    '     GridViewSearch.Rows(i).Cells(23).Enabled = False
                    If GridViewSearch.Rows(i).Cells(21).Text = "paid" Then
                        GridViewSearch.Rows(i).Cells(5).Enabled = False

                        GridViewSearch.Rows(i).Cells(3).Enabled = False
                        GridViewSearch.Rows(i).Cells(3).ToolTip = "Final Payment Paid"
                        GridViewSearch.Rows(i).Cells(3).ForeColor = Drawing.Color.Gray

                        If GridViewSearch.Rows(i).Cells(22).Text <> "Insurance" Then
                            GridViewSearch.Rows(i).Cells(5).Enabled = False
                            GridViewSearch.Rows(i).Cells(5).ForeColor = Drawing.Color.Gray
                            GridViewSearch.Rows(i).Cells(5).ToolTip = "Final Paid"
                        End If
                    ...
                    Else
                        ...
                        If GridViewSearch.Rows(i).Cells(22).Text <> "Insurance" Then
                            GridViewSearch.Rows(i).Cells(5).Enabled = True
                            GridViewSearch.Rows(i).Cells(5).ForeColor = Drawing.Color.Gray
                            GridViewSearch.Rows(i).Cells(5).ToolTip = "Not an Entitled Patient"
                        End If

                    End If
                    GridViewSearch.Rows(i).Cells(21).Visible = False

                    GridViewSearch.Rows(i).Cells(8).Text = GridViewSearch.Rows(i).Cells(8).Text
                    GridViewSearch.Rows(i).Cells(9).Text = GridViewSearch.Rows(i).Cells(9).Text & " " & GridViewSearch.Rows(i).Cells(10).Text & " " & GridViewSearch.Rows(i).Cells(11).Text
    ...
    Protected Sub GridViewSearch_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridViewSearch.RowCommand
        If e.CommandName = "ViewAdvances" Then
            Session.Add("registrationNo", GridViewSearch.Rows(e.CommandArgument).Cells(8).Text)
            Session.Add("Payid", GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
            Response.Redirect("~/Patient Billing/Approved_refund.aspx?regno=" & GridViewSearch.Rows(e.CommandArgument).Cells(8).Text & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
        ElseIf e.CommandName = "View" Then
            Session.Add("registrationNo", GridViewSearch.Rows(e.CommandArgument).Cells(8).Text)
            Session.Add("Payid", GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
            Response.Redirect("~/Patient Billing/Opd_Slip_Report_Private.aspx?regNo=" & GridViewSearch.Rows(e.CommandArgument).Cells(8).Text & "&yealyNo=" & GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
        ...
        ElseIf e.CommandName = "Cencel" Then
            ...
            Response.Redirect("~/Patient Registration/CardPage.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(8).Text, True) & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
        ...
        ElseIf e.CommandName = "Editrecord" Then
            ...
            Response.Redirect("Update_Patient.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True) & "&payid=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(5).Text, True))
        ElseIf e.CommandName = "patienttrack" Then
            Response.Redirect("PatientHistory.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True) & "&payid=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(5).Text, True))
        ElseIf e.CommandName = "Medicine" Then
            Session.Add("registrationNo", ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(8).Text, True))
            Session.Add("Yearlyno", GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)

            Response.Redirect("~/Pharmacy/Medicine_Consumption_To_WIPatient.aspx")
        ElseIf e.CommandName = "Services" Then
            ...
            Session.Add("registrationNo", ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(8).Text, True))
            Session.Add("YearlyNo", GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
            'Response.Write(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
            'Response.End()
            Dim dt As DataTable = New DataTable()
            Try
                Dim con_str As String = ConfigurationManager.ConnectionStrings("Basic_Data_ConnectionString").ConnectionString
                Dim con_basic As SqlConnection = New SqlConnection(con_str)
                Dim command_basic As SqlCommand = New SqlCommand("Select Payment.Billing_party_id from Patient inner join Payment on Payment.RegNo=Patient.RegNo where payment.RegNo=@regno and Payment.PayID=@payid", con_basic)
                command_basic.Parameters.AddWithValue("@regno", ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(8).Text, True))
                command_basic.Parameters.AddWithValue("@payid", GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
                Dim sda As SqlDataAdapter = New SqlDataAdapter(command_basic)

                sda.Fill(dt)
                If (dt.Rows.Count > 0) And Not (dt.Rows(0)(0) = "0") Then


                    Session.Add("PatientBillingParty_Id", dt.Rows(0)(0))
                End If
            Catch ex As Exception

            End Try
            If dt.Rows.Count > 0 And Not (dt.Rows(0)(0) = "0") Then
                'Response.Redirect("Services_Page_Panel.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(8).Text, True) & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(8).Text)


                Response.Redirect("Services_Page_Panel.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(8).Text, True) & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
            ...
            '    Session.Add("Dept_Service", Get_DeptID(ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True), GridViewSearch.Rows(e.CommandArgument).Cells(5).Text))
            '    'Response.Write(Session("Dept_Service") & "dfsdfsfd")
            '    Session.Add("registrationNo", ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True))
            '    Session.Add("YearlyNo", GridViewSearch.Rows(e.CommandArgument).Cells(5).Text)
            '    Response.Redirect("Patient_Package_Service.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True) & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(5).Text)
        ElseIf e.CommandName = "OPDSLIP" Then
            ...
            '    Session.Add("registrationNo", ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True))
            '    Session.Add("YearlyNo", GridViewSearch.Rows(e.CommandArgument).Cells(5).Text)
            '    Response.Redirect("CencelRegistration.aspx?regno=" & ENC.Encrypt_Main(GridViewSearch.Rows(e.CommandArgument).Cells(7).Text, True) & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(5).Text)
        ElseIf e.CommandName = "AddLetter" Then
            ...
            'Session.Add("registrationNo", GridViewSearch.Rows(e.CommandArgument).Cells(8).Text)
            'Session.Add("YearlyNo", GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
            'Response.Redirect("~/Patient Billing/Party_Letter.aspx?regno=" & GridViewSearch.Rows(e.CommandArgument).Cells(8).Text & "&payid=" & GridViewSearch.Rows(e.CommandArgument).Cells(7).Text)
    ...
    Protected Sub GridViewSearch_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridViewSearch.RowDataBound
        '    If Session("dept_id").ToString() = "14" Then
