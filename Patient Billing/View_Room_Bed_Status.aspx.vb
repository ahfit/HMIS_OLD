...
Partial Class Patient_Billing_View_Room_Bed_Status
    ...
    Protected Sub btnallbill_Click(sender As Object, e As EventArgs)
        Dim url As String
        url = ("../Patient%20Billing/AllPatientServicesRuningBillReport.aspx")
    ...
    Protected Sub imgClose_Click(sender As Object, e As ImageClickEventArgs)
        panel.Visible = False
    End Sub
    Protected Sub gvClearanceStatus_RowDataBound(sender As Object, e As GridViewRowEventArgs)
        If (e.Row.RowType = DataControlRowType.DataRow) Then

            Dim hfd As HiddenField = e.Row.FindControl("hfdforcolor")
            Dim lbl As Label = e.Row.FindControl("lblColorStatus")

            If (hfd.Value = "Yellow") Then
                e.Row.BackColor = System.Drawing.Color.Yellow
            ElseIf (hfd.Value = "Green") Then
                e.Row.BackColor = System.Drawing.Color.Green
            End If

        End If
    End Sub
    Protected Sub btnClearanceStatus_Click(sender As Object, e As EventArgs)
        Dim lnkbtn As LinkButton = sender
        hfdRegNo.Value = lnkbtn.CommandArgument
        fmvPatientInfo.DataBind()
        panel.Visible = True
        Dim con As SqlConnection = New SqlConnection(constr)
        Dim command As SqlCommand = New SqlCommand("usp_SelectPatientClearanceStatus", con)
        command.CommandType = CommandType.StoredProcedure
        command.Parameters.AddWithValue("@yearlyno", lnkbtn.ToolTip)
        Dim sda As SqlDataAdapter = New SqlDataAdapter(command)
        Dim dt As DataTable = New DataTable()
        sda.Fill(dt)
        gvClearanceStatus.DataSource = dt
        gvClearanceStatus.DataBind()
    ...
    Private Sub get_BedSummary()
        ...
        Try
            con.Open()
            cmd = New SqlCommand("usp_BedStatusWardWiseNewWard", con)
            cmd.CommandType = CommandType.StoredProcedure
    ...
    Private Sub get_BedDetail()
        ...
        Try
            con.Open()
            cmd = New SqlCommand("usp_getRoomBedStatusWard", con)
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("@DeptId", DDL_Department.SelectedValue)
            cmd.Parameters.AddWithValue("@Ward_id", DropDownListFloor.SelectedValue)
    ...
    Protected Sub lnkbtnDischarge_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            SqlDataSourceUpdate.UpdateParameters("bedid").DefaultValue = bedid.Value
    ...
    Protected Sub GridViewTest_PreRender(sender As Object, e As System.EventArgs) Handles GridViewTest.PreRender
        ...
        If GridViewTest.Rows.Count > 0 Then
            ...
            For i = 0 To GridViewTest.Rows.Count - 1
                ...
                Dim lnkService As LinkButton = GridViewTest.Rows(i).FindControl("btnServicees")
                Dim btnRunningbills As LinkButton = GridViewTest.Rows(i).FindControl("btnRunningbill")

                Dim lnkChangeRoom As LinkButton = GridViewTest.Rows(i).FindControl("lnk_ChageRoom")
                Dim lnkNursingNotes As LinkButton = GridViewTest.Rows(i).FindControl("lnk_NursingNotes")
                Dim lnkBedTransfer As LinkButton = GridViewTest.Rows(i).FindControl("lnk_PanelBill")
                Dim btnClearanceStatus As LinkButton = GridViewTest.Rows(i).FindControl("btnClearanceStatus")
                ...
                If Convert.ToInt32(Session("emp_id").ToString()) = 385 Or Convert.ToInt32(Session("emp_id").ToString()) = 3 Or Convert.ToInt32(Session("emp_id").ToString()) = 386 Then
                    ...
                    btnClearanceStatus.Visible = False
                End If
                ...
                If link.ToolTip = "" Then
                    ...
                    lnkChangeRoom.Visible = False
                    btnRunningbills.Visible = False
                    lnkBedTransfer.Visible = False
                    lnkNursingNotes.Visible = False
                End If
    ...
    Protected Sub lnk_PacKage_Click(sender As Object, e As System.EventArgs)
        ...
        Dim url As String
        url = "../Pharmacy/OT_Medicine_issue_item_wise.aspx?Reg_No=" + lk.CommandArgument + "&Yearly_No=" + lk.ToolTip
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)

        'Pharmacy/OT_Medicine_issue_item_wise.aspx?regno=00-01-14&YearlyNo=36
    ...
    Protected Sub btnServicees_Click(sender As Object, e As EventArgs)
        ...
        'Dim yearlyNo As HiddenField = Row.FindControl("Hf_Yearly")
        Dim url As String
        url = ("../Pharmacy/OT_Services_Page.aspx?RegNo=" + lk.CommandArgument + "&payid=" + lk.ToolTip)
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)
    ...
    Protected Sub lnk_ChageRoom_Click(sender As Object, e As EventArgs)
        Dim lk As LinkButton = sender
        Dim Row As GridViewRow = lk.NamingContainer
        Session.Remove("RegistrationNo")
        Session.Remove("YearlyNo")
        ...
        Session("RegistrationNo") = lk.CommandArgument
        Session("YearlyNo") = lk.ToolTip
        Dim HfPatientType As HiddenField = Row.FindControl("HfPatientTypeId")
        Dim hfdbedid As HiddenField = Row.FindControl("hf_BedID")

        hfCurrbedid.Value = hfdbedid.Value
        'DropDownListPType.DataBind()
        'DropDownListPType.SelectedValue = HfPatientType.Value
        Dim wrdid As HiddenField = Row.FindControl("hf_WardID")
        hfwardid.Value = wrdid.Value
        ...
        'Dim url As String
        'url = ("ChangeRoom.aspx?PT=" + HfPatientType.Value)
        'Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)
    End Sub
    Protected Sub lnk_NursingNotes_Click(sender As Object, e As EventArgs)
        Dim lk As LinkButton = sender
        Dim Row As GridViewRow = lk.NamingContainer
        Session.Remove("RegistrationNo")
        Session.Remove("YearlyNo")
        ...
        Dim url As String
        url = ("../Nursing/Nursing_Notes.aspx?regno=" + lk.CommandArgument + "&PayID=" + lk.ToolTip)
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)
    End Sub
    ...
    Protected Sub btnRunningbill_Click(sender As Object, e As EventArgs)
        ...
        Dim url As String
        url = ("../Patient Billing/PatientServicesRuningBillReportForBed.aspx?RegNo=" + lk.CommandArgument + "&YearlyNo=" + lk.ToolTip)
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)



    End Sub
    Protected Sub loadAdmissionType()
        Try
            Dim con_bed As SqlConnection = New SqlConnection(constr)
            Dim command1 As SqlCommand = New SqlCommand("SELECT   Id, AdmissionType FROM  [Registration].[dbo].[AdmissionType]", con_bed)
            Dim sda As SqlDataAdapter = New SqlDataAdapter(command1)
            Dim dt As DataTable = New DataTable()
            sda.Fill(dt)
            DDLAdmissionType.DataSource = dt
            DDLAdmissionType.DataTextField = "AdmissionType"
            DDLAdmissionType.DataValueField = "AdmissionType"
    ...
    Protected Sub btnupdate_Click(sender As Object, e As EventArgs)
        Try
            Dim con_bed As SqlConnection = New SqlConnection(constr)
            Dim command1 As SqlCommand = New SqlCommand("update Registration.dbo. PT_WardAdmission set Admission_Type=@admtype  where RegNo=@regno and YearlyNo=@yearlyno", con_bed)
            Dim sda As SqlDataAdapter = New SqlDataAdapter(command1)
            command1.Parameters.AddWithValue("@regno", hfdreg.Value)
            command1.Parameters.AddWithValue("@yearlyno", hfdvisit.Value)
            command1.Parameters.AddWithValue("@admtype", DDLAdmissionType.SelectedValue)
            con_bed.Open()


            command1.ExecuteNonQuery()
            con_bed.Close()
            Pnlchangetype.Visible = False
            get_BedDetail()
        Catch ex As Exception
        End Try
    End Sub

    Protected Sub lnktype_Click(sender As Object, e As EventArgs)
        Dim lnk As LinkButton = sender
        Dim reg As HiddenField = lnk.FindControl("hf_regno")
        Dim vist As HiddenField = lnk.FindControl("Hf_Yearly")

        hfdreg.Value = reg.Value
        hfdvisit.Value = vist.Value
        loadAdmissionType()
        Pnlchangetype.Visible = True
         
    End Sub
    Public Sub ChagneBed(ByVal bedid As Integer)
        Try
            Dim con_bed As SqlConnection = New SqlConnection(constr)
            Dim command1 As SqlCommand = New SqlCommand("uspChangeBedCurrent", con_bed)
            command1.CommandType = CommandType.StoredProcedure
            Dim sda As SqlDataAdapter = New SqlDataAdapter(command1)
            command1.Parameters.AddWithValue("@wardid", DropDownList2.SelectedValue)
            command1.Parameters.AddWithValue("@Empid", Session("Emp_ID"))
            command1.Parameters.AddWithValue("@regno", Session("RegistrationNo"))
            command1.Parameters.AddWithValue("@yearlyno", Session("YearlyNo"))
            command1.Parameters.AddWithValue("@bedid", hfCurrbedid.Value)
            command1.Parameters.AddWithValue("@currentbedid", bedid)
            ...
            command1.ExecuteNonQuery()
            con_bed.Close()
            Lbl_Mg.Text = "Bed Assigned Sucessfully"
            Pnlchangetype.Visible = False
    ...
    Protected Sub shift()
        Try
            'If CheckBoxListRoom.Items.Count > 0 Then
            '    Dim Flag_room As Boolean = False
            '    For i As Integer = 0 To CheckBoxListRoom.Items.Count - 1
            '        If CheckBoxListRoom.Items(i).Selected = True Then
            '            Flag_room = True
            '            ChagneBed(CheckBoxListRoom.Items(i).Selected)
            '            Return
            '        End If
            '    Next

            If RadioButtonListBed.Items.Count > 0 Then
                Dim Flag_room As Boolean = False
                For i As Integer = 0 To RadioButtonListBed.Items.Count - 1
                    If RadioButtonListBed.Items(i).Selected = True Then
                        Flag_room = True
                        ChagneBed(RadioButtonListBed.Items(i).Value)
                        Return
                    End If
                Next
                If Flag_room = False Then
                    Lbl_Mg.Text = "Must Select Bed"
                    Exit Sub
                End If
            End If
        Catch ex As Exception
    ...
    Protected Sub btnclosepanel_Click(sender As Object, e As EventArgs)
        pnlchangeroom.Visible = False
        get_BedDetail()
    End Sub
End Class
