...
Partial Class Patient_Registration_Services_Page
    ...
    Dim T_S_Dis As Double = 0, T_M_Dis As Double = 0, Total_Patient As Double = 0
    ...
    Protected Sub GridView_Date_Wise_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView_Date_Wise.RowDataBound
        Try
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                Dim txtPanelAmount As TextBox = DirectCast(e.Row.FindControl("txtPanelAmount"), TextBox)
                Dim lblPatientAmount As Label = DirectCast(e.Row.FindControl("lblPatientValue"), Label)
                If Convert.ToDecimal(txtPanelAmount.Text) > 0 Then
                    txtPanelAmount.Text = coma.place_Coma(txtPanelAmount.Text)
                End If
                If Convert.ToDecimal(lblPatientAmount.Text) > 0 Then
                    lblPatientAmount.Text = coma.place_Coma(lblPatientAmount.Text)
                End If
                Total_panel += Convert.ToDecimal(txtPanelAmount.Text)
                Total_Patient += Convert.ToDecimal(lblPatientAmount.Text)
            ...
            ElseIf e.Row.RowType = DataControlRowType.Footer Then
                ...
                If Total_panel > 0 Then
                    e.Row.Cells(8).Text = coma.place_Coma((Total_panel).ToString())
                    'Else
                    '    e.Row.Cells(8).Text = coma.place_Coma(((total + Total_panel)).ToString())
                End If
                If Total_Patient > 0 Then
                    e.Row.Cells(9).Text = coma.place_Coma((Total_Patient).ToString())
                    'Else
                    '    e.Row.Cells(9).Text = coma.place_Coma(((total + Total_Patient)).ToString())
                End If
            End If
        Catch ex As Exception
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            '    GridView_Date_Wise.DeleteRow(index)btnSaveCheckedServices
            'Next
            ...
            'Dim val As String = DropDownList_Admin_Service_Package.SelectedValue
            'Dim val2 As String = DropDownList1.SelectedValue
            'Dim r As String = Request.QueryString("regno")
            'Dim p As String = Request.QueryString("payid")
            'Dim val3 As String = DropDownList_Admin_Service_Package.SelectedValue
            'Dim val4 As String = Session("HospitalID")
            'Dim val5 As String = Session("Patient_Type_Id")
            'Dim val6 As String = Session("IPDOPD")
            Dim con_str1 As String = ConfigurationManager.ConnectionStrings("Basic_Data_ConnectionString").ConnectionString
            Dim con_basic1 As SqlConnection = New SqlConnection(con_str1)
            Dim command_basic1 As SqlCommand = New SqlCommand("Select Payment.Billing_party_id from Patient inner join Payment on Payment.RegNo=Patient.RegNo where payment.RegNo=@regno and Payment.PayID=@payid", con_basic1)
            command_basic1.Parameters.AddWithValue("@regno", Request.QueryString("regno"))
            command_basic1.Parameters.AddWithValue("@payid", Request.QueryString("payid"))
            Dim sda1 As SqlDataAdapter = New SqlDataAdapter(command_basic1)
            Dim dt1 As DataTable = New DataTable()
            sda1.Fill(dt1)
            If ((dt1.Rows.Count > 0) And (Not (dt1.Rows(0)(0) = "0"))) Then
                Session("BillingParty_Id") = dt1.Rows(0)(0)
            Else
                Session("BillingParty_Id") = "0"
            End If
            GridView2.DataBind()
                ...
                command_basic.Parameters.AddWithValue("@regno", Request.QueryString("regno"))
                command_basic.Parameters.AddWithValue("@payid", Request.QueryString("payid"))
                ...
                If ((dt.Rows.Count > 0) And (Not (dt.Rows(0)(0) = "0"))) Then
                    ...
                    Session("Patient_Type_ID") = 4
                    ' If (GridView_Date_Wise.Rows.Count > 0) And (Session("Patient_Type_ID") = 4) Then
                    If (Session("Patient_Type_ID") = 4) Then
                        pnlAmountDis.Visible = True
                        getPatientLimit()
                        If txtAmountDis.Text <> "0" Then
                            txtAmountDis.Enabled = False
                        End If
                    Else
    ...
    Protected Sub Search(ByVal reg As String)
        ...
        Response.Redirect("~/Patient Billing/Services_Page_Panel.aspx?regno=" + reg + "&payid=" + Visit_No)
    ...
    Protected Sub Insert_services(ByVal index As Integer)
        Try
            ...
            If HiddenField_Previous_index.Value <> index Then
                ...
                Dim Hid_Psid As HiddenField = GridView2.Rows(index).FindControl("hdfPaSID")
                ...
                If HiddenField_Flag.Value = 0 Then
                    ...
                    If HiddenField_amount.Value <> "" Then
                        ...
                        If Not IsNothing(Hid_Psid) Then
                            SqlDataSource_For_Grid.InsertParameters("SubID").DefaultValue = Hid_Psid.Value
                        Else
                            SqlDataSource_For_Grid.InsertParameters("SubID").DefaultValue = 0
                        End If
    ...
    Protected Sub btnUpdateServices_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnUpdateServices.Click
        ...
        For i As Integer = 0 To GridView_Date_Wise.Rows.Count - 1
            ...
            Dim Command As SqlCommand = New SqlCommand("UpdatePanelPatientBill", con)
            Command.CommandType = CommandType.StoredProcedure
            Command.Parameters.AddWithValue("@PsId", PSId.Value)
            Command.Parameters.AddWithValue("@PanelAmount", panelAmount.Text.Replace(",", ""))
    ...
    Protected Sub btnPrevious_Click(sender As Object, e As EventArgs) Handles btnPrevious.Click
        Response.Redirect("Services_Page_Panel.aspx?regno=" + Request.QueryString("regno") + "&payid=" + Request.QueryString("payid"))
    End Sub
    ...
    Protected Sub btnDivide_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDivide.Click
        Dim totalServicesAmount As Double = 0, TotalSysDiscount As Double = 0
        For Each gvRow As GridViewRow In GridView_Date_Wise.Rows
            totalServicesAmount += Convert.ToDouble(gvRow.Cells(3).Text)
            TotalSysDiscount += Convert.ToDouble(gvRow.Cells(4).Text)
        Next
        If txtAmountDis.Text <> "" Then
            Label_msg.Visible = False
            If Convert.ToDouble(txtAmountDis.Text) <= (totalServicesAmount - TotalSysDiscount) Then
                ...
                TotalSysDiscount = 0
            Else
                Label_msg.Text = "Enter Correct Amount not greater than = " + coma.place_Coma((totalServicesAmount - TotalSysDiscount).ToString())
                Label_msg.Visible = True
                txtAmountDis.Focus()
            End If
        Else
            Label_msg.Text = "Enter Amount"
            Label_msg.Visible = True
            txtAmountDis.Focus()
        End If
    ...
    Protected Sub btnShowLimitPage_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnShowLimitPage.Click
        Dim url As String = "PanelCompanyCashLimit.aspx?RegNo=" + Request.QueryString("regno")
        Page.ClientScript.RegisterStartupScript(Me.GetType, "OpenWindow", "window.open('" + url + "','_newtab');", True)

    End Sub
    Protected Sub getPatientLimit()
        Dim dbm As DbManager = New DbManager()
        Dim sp As SqlParameter() = {New SqlParameter("@RegNo", Request.QueryString("regno"))}
        Dim dt As DataTable = New DataTable()
        dt = dbm.ExecuteDataTable("uspGetPatientTotalLimit", "BasicDataConnectionString", sp)
        If dt.Rows.Count > 0 Then
            txtAmountDis.Text = dt.Rows(0)(0).ToString()
        Else
            txtAmountDis.Text = "0"
        End If
