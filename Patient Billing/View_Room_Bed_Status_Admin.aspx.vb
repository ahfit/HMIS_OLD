...
Partial Class Patient_Billing_View_Room_Bed_Status_Admin
    ...
    Protected Sub lnk_btn_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim url As String
        url = ("../Patient Billing/Services_Page.aspx?RegNo=" + lk.CommandArgument + "&payid=" + lk.ToolTip)
    ...
    Protected Sub lnkbtnDischarge_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            Dim LinkButton_Patient As LinkButton = Row.FindControl("LinkButton_Patient")
            lblMrNo.Text = reg.Value
            lblName.Text = LinkButton_Patient.Text
    ...
    Protected Sub GridViewTest_PreRender(sender As Object, e As System.EventArgs) Handles GridViewTest.PreRender
        ...
        If GridViewTest.Rows.Count > 0 Then
            ...
            For i = 0 To GridViewTest.Rows.Count - 1
                ...
                Dim btnClearance As LinkButton = GridViewTest.Rows(i).FindControl("btnClearanceStatus")
                ...
                If Not String.IsNullOrEmpty(hfDischarge.Value) Then
                    ...
                    If hfDischarge.Value = False Then
                        '  lnkbtnDischarge.Visible = False
                        ' GridViewTest.Rows(i).Enabled = False
    ...
    Protected Sub Button_Report_Click(sender As Object, e As EventArgs)
        Try
            If Not String.IsNullOrEmpty(yearlyNo.Value) Then
                'Dim con As SqlConnection = New SqlConnection(constr)
                'Dim command As SqlCommand = New SqlCommand("Patient_Discharge", con)
                'command.CommandType = CommandType.StoredProcedure
                'command.Parameters.AddWithValue("@PayID", yearlyNo.Value)
                ...
                SqlDataSourceUpdate.Update()
                'Dim url As String = ""
                'url = "Patient Billing/Patient_Discharge_Slip.aspx?PayID=" + yearlyNo.Value
                'Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", ("window.open(\'" + (url + "\',\'_newtab\');")), True)
                get_BedSummary()
                get_BedDetail()
                Response.Redirect("~/Patient Billing/Patient_Discharge_Slip.aspx?PayID=" + yearlyNo.Value)
    ...
    Protected Sub btnCollectionSummary_Click(sender As Object, e As EventArgs)
        Dim lk As LinkButton = sender
        Dim Row As GridViewRow = lk.NamingContainer
        Session.Remove("RegistrationNo")
        Session.Remove("YearlyNo")
        ...
        Dim url As String
        url = ("../Patient%20Billing/PatientSummaryCollectionForBed.aspx?RegNo=" + lk.CommandArgument + "&YearlyNo=" + lk.ToolTip)
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)
    End Sub
    ...
    Protected Sub btnPayment_Click(sender As Object, e As EventArgs)
        ...
        Dim url As String
        url = ("../Patient%20Billing/Patient_Payment.aspx?regno=" + lk.CommandArgument + "&payid=" + lk.ToolTip)
        Page.ClientScript.RegisterStartupScript(Me.GetType(), "OpenWindow", "window.open('" + url + "','_newtab');", True)
    End Sub
End Class
