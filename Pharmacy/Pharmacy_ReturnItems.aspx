<%@ Page Title="" Language="C#" MasterPageFile="~/hacims_masterpage_billing.master" AutoEventWireup="true" CodeFile="Pharmacy_ReturnItems.aspx.cs" Inherits="Pharmacy_ReturnItems" %>
    ...
    <script type="text/javascript">
        function CheckQuantityLimit(valueCmp, txtBox, txtDemandQty) {
            try {
                ...
                var maxAllow = parseInt(valueCmp) - parseInt(txtDemandQty);
                var enteredValue = parseInt(textBox.value);
                if (enteredValue > maxAllow) {
                    alert('Maximum allowed qty : ' + maxAllow);
                    textBox.value = "0";
                    textBox.focus();
                }
            }
            ...
            <tr>
                <td>
                    <table width="100%">
                        <tr>
                            ...
                            <td align="left">
                                ...
                                <asp:SqlDataSource ID="dsDepartment" runat="server" ConnectionString="<%$ ConnectionStrings:STOREConnectionString %>"
                                    SelectCommand="Select Dept_ID,Dept_Name From Department where hospital_id=@hospitalid Order By Dept_Name">
                                    <SelectParameters>
                        ...
                        <tr>
                            <td style="font-weight: bold; text-align: right; padding-right: 5px;">Remarks : </td>
                            <td align="left">
                                <asp:TextBox ID="txtRemarks" runat="server" TextMode="MultiLine" Height="50px" Width="350px"></asp:TextBox></td>
                            <br />
                        </tr>
                    </table>
                </td>
                ...
                <td align="center">
                    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False"
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="Consumed QTY" SortExpression="Consumed_QTY">
                                <ItemTemplate>
                                    <asp:HiddenField runat="server" ID="hfPO_ID" Value='<%# Eval("PO_ID") %>' />
                                    <asp:HiddenField runat="server" ID="hfPO_ID_Sub" Value='<%# Eval("PO_ID_Sub") %>' />
                                    <asp:HiddenField runat="server" ID="hfExpiryDate" Value='<%# Eval("ExpiryDate") %>' />
                                    <asp:HiddenField runat="server" ID="hfBatchNo" Value='<%# Eval("BatchNo") %>' />
                                    <asp:Label ID="lblConQty" runat="server" Text='<%# Bind("Consumed_QTY") %>'></asp:Label>
                                </ItemTemplate>
                            </asp:TemplateField>
                            <asp:TemplateField HeaderText="Qty Returned" SortExpression="QtyReturn">
                                <ItemTemplate>
                                    <asp:Label ID="lblReturnQty" runat="server" Text='<%# Bind("QtyReturn") %>'></asp:Label>
                                </ItemTemplate>
                            </asp:TemplateField>

                            <asp:TemplateField HeaderText="Reserved Qty">
                                <ItemTemplate>
                                    <asp:TextBox ID="txtQty" runat="server" Text='<%# Bind("Return_Qty") %>' Visible='<%# Eval("IsVisible") %>'></asp:TextBox>
                            ...
                            <asp:TemplateField>
                                <ItemTemplate>
                                    ...
                                    <asp:LinkButton ID="Chk_Status" runat="server" Text="Mark"
                                        CommandArgument='<%# Eval("Consumption_Detail_ID") %>' OnClick="btnChk_Status_Click"
                                        Visible='<%# Eval("IsVisible") %>'></asp:LinkButton>
                                    <br />
                                    <asp:HiddenField ID="hdfAllowSerial" runat="server" Value='<%# Eval("Allow_Serial") %>' />
                    ...
                    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:STOREConnectionString %>"
                        ProviderName="<%$ ConnectionStrings:STOREConnectionString.ProviderName %>" SelectCommand="Return_Consumption_BatchWiseItems"
            ...
            <tr>
                <td align="center">
                    ...
                    <asp:Button ID="btn_forward" runat="server" OnClick="btn_forward_Click"
                        Text="Mark Sale Return" />

                    <asp:Button ID="btnShow_return" runat="server" OnClick="btnShow_return_Click"
                        Text="Show Return Items" Visible="true" />
                    <asp:Button ID="btnShowDC" runat="server" Visible="false"
                        Text="Show Return DC" OnClick="btnShowDC_Click" />
                    ...
                    <asp:GridView BackColor="LightGray" ID="gvReturnItems" runat="server" Width="80%" Visible="False"
                        ...
                        <Columns>
                            <asp:TemplateField HeaderText="ID">
                                <ItemTemplate>
                                    <asp:Label ID="lblDetilID" runat="server" Text='<%# Eval("Consumption_Detail_ID") %>'></asp:Label>
                                </ItemTemplate>
                            </asp:TemplateField>
            ...
            <tr>
                <td align="right">
                            ...
                            <asp:TemplateField>
                                <HeaderTemplate>
                                    <asp:CheckBox ID="chkAll" runat="server" AutoPostBack="True"
                                        OnCheckedChanged="chkAll_CheckedChanged" Height="10" Width="50" />
                                </HeaderTemplate>
                                <ItemTemplate>
                    ...
                    <div style="text-align: right;">
                        <asp:Button ID="btnReturnSerials" runat="server" Text="Return Item(s)"
