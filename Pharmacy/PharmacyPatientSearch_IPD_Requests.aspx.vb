...
Partial Class Pharmacy_PharmacyPatientSearch_IPD_Requests
    ...
    Protected Sub GridViewSearch_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridViewSearch.RowCommand
        If e.CommandName <> "Sort" Then
            ...
            Dim HfIBID As HiddenField = row.FindControl("HfIBID")
            If e.CommandName = "View" Then
                Session.Add("registrationNo", regNo.Text)
                Session.Add("YearlyNo", yearlyNo.Text)
                Dim sb As New StringBuilder
                sb.Append("<script language='javascript' type='text/javascript'>window.open('../Pharmacy/Medicine_issue_item_wise.aspx?RegNo=" + regNo.Text + "&yearlyno=" + yearlyNo.Text + "&IBID=" + HfIBID.Value + "','_blank');</Script>")
    ...
    Function search_DataSet_Pharmacy() As DataTable
        ...
        ''''''''''''''''Removed Parameters''''''''''''''''''''''''''''''
        'New SqlParameter("@PatientName", TextBoxPMName),
        'New SqlParameter("@PatientName", TextBoxPLName),
        ...
        '  New SqlParameter("@RelationName", TextBoxRMName),
        'New SqlParameter("@RelationName", TextBoxRLName),
        'New SqlParameter("@request_type_id", 1),
        'New SqlParameter("@Status", 0),

        Dim sqlParams As SqlParameter() = {
        New SqlParameter("@VisitNo", YearlyNo),
        New SqlParameter("@PatientName", TextBoxPFName),
        New SqlParameter("@RegNo", TextBoxRegNo),
        New SqlParameter("@RelationName", TextBoxRFName),
        New SqlParameter("@RegDateFrom", WebDate),
        New SqlParameter("@RegDateTo", WebDate),
        ...
        New SqlParameter("@EmpID", HttpContext.Current.Session("Emp_ID")),
        New SqlParameter("@NicNo", TextBoxCNIC),
        New SqlParameter("@MobileNo", TextBoxPhone)
}

        Dim search_DSPharmacy As DataTable = New DataTable()
        Dim objManager As DbManager = New DbManager()

        search_DSPharmacy = objManager.ExecuteDataTable("Search_Patient_For_ReceptionistIPD_Requests", "Reg_ConnectionString", sqlParams)
