<%@ Page Language="VB" AutoEventWireup="false" MasterPageFile="~/hacims_masterpage_admin.master" CodeFile="Employee_OverTime.aspx.vb" Inherits="Payroll_Employee_OverTime" EnableEventValidation="false" Theme="theme_hacims" ViewStateEncryptionMode="Never" MaintainScrollPositionOnPostback="true" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="bxmain inner_content " style="width: 100%">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl_form">
            <tr>
                <td width="16%" align="right">Department : </td>
                <td width="25%">
                    <asp:DropDownList ID="Dropdownlistdepartment" runat="server"
                        DataSourceID="SqlDataSourceDepartment" DataTextField="SubDept_Name" AutoPostBack="true" OnSelectedIndexChanged="Dropdownlistdepartment_SelectedIndexChanged"
    ...
    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" CssClass="Grid_1" Width="100%" DataKeyNames="Emp_Id">
        <Columns>
            ...
            <asp:BoundField DataField="overtime" HeaderText="Over Time" SortExpression="overtime" />          
                         <asp:BoundField DataField="salary" HeaderText="Over time Amount" SortExpression="amount"  ReadOnly="true" />
            ...
            <asp:HyperLinkField DataNavigateUrlFields="month,Emp_Id,year" DataNavigateUrlFormatString="~/Machine_Attendance/EmpoyeeDayWiseOverTime.aspx?month={0}&amp;Emp_ID={1}&amp;year={2}"
    ...
    <asp:SqlDataSource ID="SqlDataSourceDepartment" runat="server" ConnectionString="<%$ ConnectionStrings:Basic_Data_ConnectionString %>"
        ...
        SelectCommand="SELECT 0 as SubDept_Id, 'All' as SubDept_Name UNION Select SubDept_Id,SubDept_Name from SubDepartment ORDER BY [SubDept_Name]"></asp:SqlDataSource>
    <asp:SqlDataSource ID="SqlDataSourceEmployee" runat="server" ConnectionString="<%$ ConnectionStrings:Basic_Data_ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:Basic_Data_ConnectionString.ProviderName %>"
        SelectCommand=" Select 'ALL' as Name ,0 as EmpID union SELECT ISNULL(Employee.EFName, ' ') + ' ' + ISNULL(Employee.EMName, ' ') + ' ' + ISNULL(Employee.ELName, ' ') AS Name, EmpID from Employee
 
Where (Employee.SubDeptId=@SubDept_Id or @SubDept_Id=0) order by Name">
        <SelectParameters>
            <asp:ControlParameter ControlID="Dropdownlistdepartment" Name="SubDept_Id" PropertyName="SelectedValue" />
         </SelectParameters>
    </asp:SqlDataSource>
