...
Partial Class Payroll_EmployeeAllowanceDeductionReport
    ...
    Private Sub ShowReport()
        ...
        Try
            con.Open()
            cmd = New SqlCommand("usp_GetAllSalaryHeads", con)
            ...
            cmd.Parameters.AddWithValue("@IsVariable", ddlHeadType.SelectedValue)
            cmd.Parameters.AddWithValue("@HeadType", ddlAllowanceDeduction.SelectedValue)
            Dim da As SqlDataAdapter = New SqlDataAdapter(cmd)
            Dim ds As DataSet = New DataSet()
            da.Fill(ds)
            ReportViewer1.LocalReport.ReportPath = Server.MapPath("EmployeeAllowanceDeductionReport.rdlc")
            ReportViewer1.LocalReport.DataSources.Clear()
            Dim repParams As ReportParameter() = New ReportParameter(1) {}
            repParams(0) = New ReportParameter("Type", ddlHeadType.SelectedItem.Text)
            repParams(1) = New ReportParameter("HeadType", ddlAllowanceDeduction.SelectedItem.Text)
