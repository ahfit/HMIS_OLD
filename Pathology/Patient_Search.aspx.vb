...
Partial Class Patient_Search
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Dim Table_Element_Name() As String = {"First Name :", "Middle Name :", "Last Name :", "Relation Name :", "Relation Middle Name :", "Relation Last Name :", "Registration No. :", "Yearly No. :", "Age :", "Registration Date :", "Status :", "Test Group :", "Test Name :"}
            Dim Table_Element_Type() As String = {"TextBox", "TextBox", "TextBox", "TextBox", "TextBox", "TextBox", "TextBox", "TextBox", "TextBox", "WebDateChooser", "DropDownList_Status", "DropDownList_Test_Group", "DropDownList_Test_Name"}
            Dim Table_Element_ID() As String = {"TextBoxPFName", "TextBoxPMName", "TextBoxPLName", "TextBoxFName", "TextBoxFMName", "TextBoxLName", "TextBoxRegNo", "TextBoxYearlyNo", "TextBoxAge", "WebDateChooser1", "DropDownList_Status", "DropDownList_Test_Group", "DropDownList_Test_Name"}
            ...
            'End If
            If Not IsPostBack Then
                WebDateChooser_FromDate.Value = Date.Now.AddDays(-12)
                WebDateChooser_Todate.Value = Date.Now
                HiddenField_FromDate.Value = CDate(WebDateChooser_FromDate.Value).ToString("dd/MM/yyyy")
                HiddenField_ToDate.Value = CDate(WebDateChooser_Todate.Value).ToString("dd/MM/yyyy")
                Draw_Status_DropdownList()
                Draw_DropDownList_Test_Group()
                Draw_DropDownList_Test_Name()
                Dim TG_ID As String = Request.QueryString("TGID")
                If Not String.IsNullOrEmpty(TG_ID) Then
                    DropDownList_Test_Group.SelectedValue = TG_ID
                End If
            End If


            ' searchpanel1.search_Panel_New(Panel2, Table_Columns, Table_Element_Name, Table_Element_Type, Table_Element_ID)
            'searchpanel1.Draw_DropDownList_Test_Name("DropDownList_Test_Name", 150, Panel2)
            If Page.IsPostBack = False Then
                If DropDownList_Status.SelectedItem.Text = "" Then

                    TextBoxSample.Visible = False
                    lbl_Sample.Visible = False
                Else
                    TextBoxSample.Visible = True
    ...
    Sub Grid_Col(ByVal BTN_TXT As String, ByVal CMD As String, ByVal NextPage As String)
        Dim str_reg(GridView2.Rows.Count - 1) As String
        Dim str_yearly(GridView2.Rows.Count - 1) As String
        Dim str_main(GridView2.Rows.Count - 1) As String
        Dim str_name(GridView2.Rows.Count - 1) As String
        For i As Integer = 0 To GridView2.Rows.Count - 1
            Dim lnk_btn As LinkButton = GridView2.Rows(i).FindControl("LinkButton1")
            lnk_btn.Text = BTN_TXT
            lnk_btn.CommandName = CMD
            Dim status As String = DropDownList_Status.SelectedValue
            If status = 16 Then
                status = 6
            End If

            lnk_btn.PostBackUrl = NextPage + "?Reg_No=" + GridView2.Rows(i).Cells(2).Text + "&YearlyNo=" + GridView2.Rows(i).Cells(15).Text + "&Main_ID=" + GridView2.Rows(i).Cells(14).Text + "&sample_No=" + GridView2.Rows(i).Cells(1).Text + "&status=" + status
            str_reg(i) = GridView2.Rows(i).Cells(2).Text
            str_yearly(i) = GridView2.Rows(i).Cells(15).Text
            str_main(i) = GridView2.Rows(i).Cells(14).Text
            str_name(i) = GridView2.Rows(i).Cells(3).Text + " " + ENC.Encrypt_Main(GridView2.Rows(i).Cells(4).Text, False) + " " + ENC.Encrypt_Main(GridView2.Rows(i).Cells(5).Text, False)
        ...
        Next
        Session.Add("reg", str_reg)
        Session.Add("yearly", str_yearly)
        Session.Add("main", str_main)
        Session.Add("name", str_name)
    ...
    Protected Sub GridView2_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView2.PreRender
        Try
            If GridView2.Rows.Count > 0 Then
                ...
                If DropDownList_Status.SelectedItem.Text = "Sample Collection" Then
                    GridView2.HeaderRow.Cells(1).Visible = False

                Else
                    GridView2.HeaderRow.Cells(1).Visible = True

                End If

                GridView2.HeaderRow.Cells(1).Text = "Sample No."
                GridView2.HeaderRow.Cells(2).Text = "MR No."
                GridView2.HeaderRow.Cells(3).Text = "Patient"
                GridView2.HeaderRow.Cells(10).Text = "Requested Date‎"
                ...
                For i As Integer = 0 To GridView2.Rows.Count - 1
                    ...
                    GridView2.Rows(i).Cells(17).Visible = False
    ...
    Sub Draw_Status_DropdownList()
        Try
            ...
            If Request.QueryString("def") Is Nothing OrElse Request.QueryString("def").ToString() = "1.2" Then
                St_id = "%"
            Else
                St_id = Request.QueryString("def").ToString()
                DropDownList_Status.Enabled = False
            End If


            Dim con As SqlConnection = New System.Data.SqlClient.SqlConnection(constring)
            Dim mycommand As New System.Data.SqlClient.SqlCommand("Select * from Test_Status where act_status=1 and Status_ID like @Status_id", con)

            mycommand.CommandType = Data.CommandType.Text

            mycommand.Connection.Open()
            mycommand.Parameters.AddWithValue("@Status_id", St_id)
    ...
    Sub Draw_DropDownList_Test_Group()
        ...
        Dim constr_dept As String = ConfigurationManager.ConnectionStrings("Path_ConnectionString").ConnectionString
        Dim con_dept As SqlConnection = New SqlConnection(constr_dept)

        'Dim commnad_dept As SqlCommand = New SqlCommand("SELECT TGID, TGName FROM TestGroup", con_dept)
        Dim commnad_dept As SqlCommand = New SqlCommand("SELECT TGID, TGName FROM TestGroup", con_dept) ' Where Dept_Id = @Dept_Id And SubDept_Id = @SubDept_Id", con_dept)
        commnad_dept.CommandType = CommandType.Text
        'commnad_dept.Parameters.Add("@Dept_Id", SqlDbtype.Int).value = Session("dept_id")
        'commnad_dept.Parameters.Add("@SubDept_Id", SqlDbtype.Int).value = Session("SubdeptId")
        con_dept.Open()
    ...
    Sub Draw_DropDownList_Test_Name()
        ...
        If HiddenField_ID.Value = "0" Or HiddenField_ID.Value = "" Then
            'commnad_dept = New SqlCommand("SELECT TID, TestName FROM Test", con_dept)
            commnad_dept = New SqlCommand("SELECT Test.TID, Test.TestName FROM Test inner join Pathology2.dbo.Test_Booking TestBooking ON Test.TID = TestBooking.tsetId", con_dept)
        Else
            'commnad_dept = New SqlCommand("SELECT TID, TestName FROM Test where TGID=" + HiddenField_ID.Value, con_dept)
            commnad_dept = New SqlCommand("SELECT Test.TID, Test.TestName FROM Test inner join Pathology2.dbo.Test_Booking TestBooking ON Test.TID = TestBooking.tsetId where TGID=" + HiddenField_ID.Value, con_dept)
        End If
        commnad_dept.CommandType = CommandType.Text
        con_dept.Open()
        Dim ds_dept As New DataSet
    ...
    Function search_Patient_New_new(ByVal pnl As Panel, ByVal procedure As String, ByVal constr As String, ByVal subdeptid As Integer) As Collection
        ...
        Dim YearlyNo1 As Int64 = 0
        Dim CNIC As String = ""
        Dim Sample_No As String = "%"
        Dim Reference_No As Integer
        Dim TextBoxTestName As String = ""
        ...
        If Not String.IsNullOrEmpty(TextBoxSample.Text) Then
            Sample_No = TextBoxSample.Text.Trim()
        Else
            Sample_No = "%"
        ...
        If TxtTestName.Text.Trim = "" Then
            TextBoxTestName = "%"
        Else
            TextBoxTestName = ENC.Encrypt_Main(ENC.TitleCase(TxtTestName.Text.ToLower), True)
            
        End If

        If TextBoxSample.Text.Trim = "" Then
            Sample_No = "%"
        Else
            Sample_No = "%" + TextBoxSample.Text.Trim + "%"
            'textboxpfname = txtbox.text
        ...
        If TextBoxMobile.Text.Trim = "" Then
            TextBoxPhone = "0"
        Else
            TextBoxPhone = TextBoxMobile.Text.Trim
        ...
        If txtReference.Text.Trim = "" Then
            Reference_No = "0"
        Else
            Reference_No = txtReference.Text.Trim
        ...
        param = mycommand.Parameters.AddWithValue("@Sample_No", Sample_No)
        ...
        param = mycommand.Parameters.AddWithValue("@ReferenceNo", Reference_No)
        param = mycommand.Parameters.AddWithValue("@patientTypeID", ddl_PatientType.SelectedValue)

        param = mycommand.Parameters.AddWithValue("@PanelType", DropDownList_Parties.SelectedValue)
        param = mycommand.Parameters.AddWithValue("@TestName", TextBoxTestName)

        param = mycommand.Parameters.AddWithValue("@to_dept", Session("dept_id"))
        param = mycommand.Parameters.AddWithValue("@to_Subdept", Session("SubdeptId"))
        If procedure = "Search_hospital_Pt_group_wise" Then
            HiddenField_FromDate.Value = Convert.ToDateTime(WebDateChooser_FromDate.Value).ToString("yyyy-MM-dd")
            HiddenField_ToDate.Value = Convert.ToDateTime(WebDateChooser_Todate.Value).ToString("yyyy-MM-dd")
            ...
            param = mycommand.Parameters.AddWithValue("@SendOut", DropDownList_SendOut.SelectedValue.ToString())

            param = mycommand.Parameters.AddWithValue("@MobileNo", TextBoxPhone)
    ...
    Protected Sub ddl_PatientType_SelectedIndexChanged(sender As Object, e As EventArgs)

        Dim value = ddl_PatientType.SelectedValue
