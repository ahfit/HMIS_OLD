<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Report_ReciverCorporate.aspx.vb"
    MasterPageFile="~/customer_masterpage.master" Inherits="Report_ReciverCorporate" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <ContentTemplate>
            <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource_Grid"
                ...
                <Columns>
                    ...
                    <asp:TemplateField>
                        <ItemTemplate>
                            <asp:CheckBox ID="CheckBox1" runat="server" Visible='<%# Eval("Chk_vis") %>'  />
    ...
    <asp:SqlDataSource ID="SqlDataSource_Grid" runat="server" ConnectionString="<%$ ConnectionStrings:Path_ConnectionString %>"
        ProviderName="<%$ ConnectionStrings:Path_ConnectionString.ProviderName %>" SelectCommand="SELECT DISTINCT  PRD.Yearly_No AS YearlyNo, PRD.Reg_no AS RegNo, Pt.PFName, Pt.PMName, Pt.PLName, Pt.Relation, ISNULL(Pt.RFName + ' ', '') + ISNULL(Pt.RMName + ' ', '') + ISNULL(Pt.RLName + ' ', '') AS PatientName, TS.Status, 
                         CONVERT(varchar, PRD.Sample_Collection_Date, 107) AS Date_Time, TG.TGName, TG.TGID, PRD.Main_ID, Dp.Dept_Name, TB.TB_Name, TB.TB_ID, ISNULL(SS.Status, 'Paid') AS P_status, CONVERT(bit, 
                         CASE WHEN isnull(SS.Status, 'Paid') = 'paid' THEN 1 ELSE 0 END) AS Chk_vis
FROM            Test_Booking AS TB INNER JOIN
                         ...
                         Department AS Dp ON PRD.From_Dept_ID = Dp.Dept_ID LEFT OUTER JOIN
