...
Imports System.Io
Partial Class Update_Patient_For_User
    ...
    Sub Patient_Info()
        Try
            ...
            da.Fill(ds, "Patient")
            Dim Pre As String = ds.Tables(0).Rows(0).Item(1).ToString.Trim()

            RBLprefix.SelectedValue = Pre
            Try
                Session.Add("yearlyno", ds.Tables(0).Rows(0).Item(33).ToString())
            Catch ex As Exception

            End Try
            textboxpfname.Text = ds.Tables(0).Rows(0).Item(2).ToString
            textboxpmname.Text = ds.Tables(0).Rows(0).Item(3).ToString
            textboxplname.Text = ds.Tables(0).Rows(0).Item(4).ToString
    ...
    End Sub
    Public Function GetResultsWithOutHyphen(ByVal input As String) As String
    ...
    Function GetPatientActiveVisit() As Boolean
        ...
        Dim Procedurename As String = "Update_PatientInfoNew"
        ...
        mycommand.Parameters.AddWithValue("@CNIC", textboxnic1.Text)
        mycommand.Parameters.AddWithValue("@MobilePhone", textboxmcode.Text)
        mycommand.Parameters.AddWithValue("@HouseNo", textboxHouseNo.Text)
    ...
    '    HiddenFieldPFName.Value = textboxpfname.Text
    '    HiddenFieldPMName.Value = textboxpmname.Text
    '    HiddenFieldPLName.Value = textboxplname.Text
    '    HiddenField_phone_no.Value = textboxphoneno.Text
    '    HiddenFieldPLName.Value = textboxHouseNo.Text.Trim()
    '    'HiddenField_mobile_no.Value = GetResultsWithOutHyphen(textboxmcode.Text)

    '    Try
    '        'Dim str() As String
    '        'str = WebDateTimeEdit_DOB.Text.Split("/")
    '        'HiddenField_DOB.Value = str(1) + "/" + str(0) + "/" + str(2)
    '        'HiddenField_cnic.Value = GetResultsWithOutHyphen(textboxnic1.Text)
    ...
    Protected Sub grdEmployeeDependents_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles grdEmployeeDependents.PreRender
        ...
        For i As Integer = 0 To grdEmployeeDependents.Rows.Count - 1
            ...
            lblPatient.Text = hfPrefix.Value + " " + hfPFName.Value + " " + hfPMName.Value + " " + hfPLName.Value
