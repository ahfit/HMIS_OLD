...
Partial Class Patient_Registration_PatientSearch
    ...
    Protected Sub GridViewSearch_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridViewSearch.RowCommand
        ...
        Try
            ...
            ElseIf e.CommandName = "patienttrack" Then
                ...
                Response.Redirect("Update_Patient_For_User.aspx?regno=" & ENC.Encrypt_Main(HiddenField_RegNo.Value, True))
    ...
    Function Get_EditRights(ByVal EmpID As Integer, ByVal Type As String) As Integer
        Try
            ...
            Dim mycommand As New System.Data.SqlClient.SqlCommand("select access from AssignTypeWiseRights where EmpID=@EmpID and Type=@Type", con)
            ...
            Dim dt As DataTable = New DataTable()
            da.Fill(dt)
            Dim EmpRights As Integer = 0
            If dt.Rows.Count > 0 Then
                EmpRights = Convert.ToInt32(dt.Rows(0)(0).ToString())
            Else
                EmpRights = 0
            End If
    ...
    Protected Sub GridViewSearch_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridViewSearch.RowDataBound
        ...
        If Get_EditRights(Convert.ToInt32(Session("Emp_ID")), "Edit Patient") = 1 Then
