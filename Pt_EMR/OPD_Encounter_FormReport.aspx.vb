...
Partial Class OPD_Encounter_FormReport
    ...
    Sub get_pt_Treatment_Pan()
        ...
        Try
            con.Open()
            Dim command As SqlCommand = New SqlCommand("select plan_id,plan_detail,type,FollowUpDays from pt_Treatment_Plan where  Reg_No=@reg_no and Yearly_No=@yearly_no and Type='Phy'", con)
            command.CommandType = CommandType.Text
            command.Parameters.AddWithValue("@reg_no", Session("RegistrationNo"))
            command.Parameters.AddWithValue("@yearly_no", Session("YearlyNo"))
            ...
            Dim palnid As String = ""
            Dim palnDetail As String = ""
            Dim palnDays As String = ""

            While reader.Read()
                palnid = reader("plan_id").ToString()
                palnDetail = reader("plan_detail").ToString()
                palnDays = reader("FollowUpDays").ToString()
            End While

            hdnPhyNotesId.Value = palnid.ToString()
            txtPhycicianNotes.Text = palnDetail.ToString()
            lblfollowupdays.Text = palnDays.ToString()
            lblfollowupnotes.Text = palnDetail.ToString()
