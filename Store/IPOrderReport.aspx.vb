...
Partial Class Store_IPO_Report
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            Try
                ...
                Dim cmd As SqlCommand = New SqlCommand("IPOReport2", con)
                ...
                If String.IsNullOrEmpty(Request.QueryString("RP")) Then
                    reportPath = Server.MapPath("~/store/Reports/IPOReport.rdlc")
                Else
                    reportPath = Server.MapPath("~/store/Reports/IPOPriceReport.rdlc")
                End If

                'Dim finManager As New FinanceDBManager(finConString)
                'Dim dsBalance As DataSet = finManager.GetPartyBalancePeriodData(DateTime.Now, PartyID, CompanyBranchID)
                'Dim dsCompanyPay As DataSet = finManager.GetPartyAndPaymentData(PartyID)

                'UpdateBalanceData(dsBalance)

                getsetTaxDiscount()
                'abc.SetParameterValue("Tax", HiddenField_Tax.Value)
                'abc.SetParameterValue("Discount", HiddenField_Discount.Value)
                'Dim repParams() As ReportParameter = New ReportParameter(8) {}
                'repParams(0) = New ReportParameter("QO_ID", HiddenFieldSO_ID.Value)
                'repParams(1) = New ReportParameter("Balance1", GetBalance(dsBalance, 1))
                'repParams(2) = New ReportParameter("Balance2", GetBalance(dsBalance, 2))
                'repParams(3) = New ReportParameter("Balance3", GetBalance(dsBalance, 3))
                'repParams(4) = New ReportParameter("Balance4", GetBalance(dsBalance, 4))
                'repParams(5) = New ReportParameter("Balance5", GetBalance(dsBalance, 5))
    ...
    Private Sub UpdateBalanceData(ByRef data As DataSet)
        ...
        'If data.Tables(0).Rows.Count > 3 Then
        '    response.write(Convert.ToDecimal(data.Tables(0).Rows(0)("Balance").ToString()))
        '    response.write(Convert.ToDecimal(data.Tables(0).Rows(1)("Balance").ToString()))
        '    response.write(Convert.ToDecimal(data.Tables(0).Rows(2)("Balance").ToString()))
        '    response.write(Convert.ToDecimal(data.Tables(0).Rows(3)("Balance").ToString()))
        '    'response.write(Convert.ToDecimal(data.Tables(0).Rows(4)("Balance").ToString()))
        '    'data.Tables(0).Rows(4)("Balance") = data.Tables(0).Rows(0)("Balance") - (data.Tables(0).Rows(1)("Balance") + data.Tables(0).Rows(2)("Balance") + data.Tables(0).Rows(3)("Balance"))
    ...
    Protected Sub getsetTaxDiscount()
        ...
        'con.Open()
        'Dim qry As String = "SELECT     Sale_Tax_Discount.Sale_ID, Tax_Discount_HeadDetail.Tax_Name, Sale_Tax_Discount.Amount FROM Tax_Discount_HeadDetail INNER JOIN Sale_Tax_Discount ON Tax_Discount_HeadDetail.Tax_Id = Sale_Tax_Discount.Tax_ID WHERE (Sale_Tax_Discount.Sale_ID = @SO_ID)"
        'Dim command As New SqlCommand(qry, con)
        'command.Parameters.AddWithValue("@SO_ID", HiddenFieldSO_ID.Value)
