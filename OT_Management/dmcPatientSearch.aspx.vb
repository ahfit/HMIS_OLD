...
Partial Class dmcPatientSearch
    ...
    Sub search(ByVal a As Integer)
        ...
        Try
            ...
            str1 = "SELECT Payment.PayID as [Yearly No], Patient.RegNo as [Registration No.], ISNULL(Patient.Prefix, ' ') + ' ' + ISNULL(Patient.PFName, ' ') + ' ' + ISNULL(Patient.PMName, ' ') + ' ' + ISNULL(Patient.PLName, ' ') AS [Patient Name], ISNULL(Patient.RFName, ' ') + ' ' + ISNULL(Patient.RMName, ' ') + ' ' + ISNULL(Patient.RLName, ' ') AS [Father's Name], Patient.Age,payment.Diagonosis as Diagnosis  FROM Payment INNER JOIN Patient ON Payment.RegNo = Patient.RegNo INNER JOIN PatientSex ON Patient.SexID = PatientSex.SexID  where patient.pfname like " + "'%'"
            Dim strcount As String = "SELECT count(payment.payid)  FROM Payment INNER JOIN Patient ON Payment.RegNo = Patient.RegNo INNER JOIN PatientSex ON Patient.SexID = PatientSex.SexID  where patient.pfname like " + "'%'"
            strcount += str
            str1 = str1 + str
    ...
    Protected Sub GridViewSearch_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridViewSearch.SelectedIndexChanged
        ' Response.Write(GridViewSearch.Rows(GridViewSearch.SelectedIndex).Cells(3).Text & " fg " & GridViewSearch.Rows(GridViewSearch.SelectedIndex).Cells(4).Text)
        Dim YearlyNo As Integer = CInt(GridViewSearch.Rows(GridViewSearch.SelectedIndex).Cells(3).Text)
        Dim Reg As String = GridViewSearch.Rows(GridViewSearch.SelectedIndex).Cells(4).Text
        Session.Add("YearlyNo", YearlyNo)
        Session.Add("RegistrationNo", Reg)
        SqlDataSourcePatient.Insert()
        'Dim b As Integer = New_Visit()
        'Response.Write(b & "" & Session("RegistrationNo"))
        Response.Redirect("dmc_selecttest.aspx?YearlyNo=" & New_Visit() & "&RegNo=" & Reg)
    End Sub
    Function New_Visit() As Integer
        Try
            ...
            Dim yearly As Integer = mycommand.Parameters("@Yearly").Value
    ...
    Sub abc()
        ...
        a.DataTextField = "Yearly No"
        GridViewSearch.Columns.Add(a)
