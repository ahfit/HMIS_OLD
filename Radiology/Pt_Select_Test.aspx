<%@ Page Language="VB" AutoEventWireup="false" ValidateRequest="false"  CodeFile="Pt_Select_Test.aspx.vb"  MasterPageFile="~/hacims_masterpage_billing.master" Inherits="Rad_Pt_Select_Test" %>
...
<asp:Parameter Name="Sample_No" Type="String"></asp:Parameter>
  ...
  <tr>
    ...
    <br />
      ...
      <fieldset class="inner_content" style=" margin-top: 0.4em;"><h2>Search</h2><table cellpadding="0" cellspacing="0" border="0" width="100%" class="tbl_form" style="margin-top:5px;" style="display:none" >
  ...
  <tr>
    ...
    <td colspan="2" align="center"><asp:Button ID="Button_Draw" runat="server" Text="Draw" Width="50px" Visible="false" />
      <asp:Button ID="Button_update" runat="server" Text="Change Status" Width="85px"  Visible="false" />
...
<br />
 ...
 <asp:DataList ID="DataList1" runat="server" DataKeyField="ID" DataSourceID="SqlDataSource_Status" Width="100%">
    ...
    <br />
        <asp:Panel ID="pnlRemraks" runat="server" Visible="false">
        <div class="inner_content bxmain"  style="width: 99%;">
                    <h2>Consultant Not Approved Remarks</h2>
                <div style="border-style: solid; border-width: 1px; height: 222px; overflow-x: hidden; overflow-y: auto;">
                    <asp:DataList ID="dlRemarks" runat="server" DataSourceID="SqlDataSourceRemarks">
                        ...
                        <ItemTemplate>
                            ...
                            <asp:Label ID="lblDate" runat="server" Text='<%# "(" + Eval("EnterDate") + ")" %>'></asp:Label>
                                </div>
                        </ItemTemplate>
                            
                    </asp:DataList>
                    <br />
                    <asp:SqlDataSource ID="SqlDataSourceRemarks" runat="server" ConnectionString="<%$ ConnectionStrings:RadiologyConnectionString %>"
                        SelectCommand="select Details AS Remarks,Convert(varchar,EnterDate,103) + ' ' + FORMAT(EnterDate,'hh:mm tt') AS EnterDate,
isnull(E.Prefix+' ','') +' ' +isnull(E.EFName+' ','') + isnull(E.EMName+' ','') + ISnull(ELName+' ','') AS Consultant from Rad_NotApproved_Test_Remarks RNA
inner join Employee E on RNA.EnterBy = E.EmpID
where RNA.MainID = @MainId and RNA.TBID = @TBID
order by EnterDate Desc">
    ...
    <td runat="server"  valign="top" style="width: 863px"> 
        ...
        ID="Button_template_Insert" Visible="false" runat="server" CssClass="btn1" Text="Insert Template" /></strong>
            ...
            <br />
                    ...
                    <asp:SqlDataSource ID="SqlDataSource_Radiology_templates" runat="server" ConnectionString="<%$ ConnectionStrings:Admin_Rad_ConnectionString %>"
                        ...
                        SelectCommand="select 0 AS Radiology_templates_ID, '--- Select Template ---' AS Templates_Name,'' AS Templates_Description, 0 AS TB_ID union
                        SELECT Radiology_templates_ID, Templates_Name, Templates_Description, TB_ID FROM Radiology_templates WHERE (Templates_Name LIKE '%' + @Templates_Name + '%') AND (TB_ID = @TB_ID)">
                        <SelectParameters>
  ...
  <tr>
    ...
    <%--<div class="clcc1Toolbar">
            <asp:TextBox ID="TextBox_Description1" runat="server" Height="500px"  TextMode="MultiLine"  Width="100%" style="width:100%; " CssClass="remarksbox" ></asp:TextBox> 
                         <br /> 
         </div> --%>
...
<div align="center" style="margin-top:10px; display:inline-table; " >
   ...
   <%-- <asp:LinkButton ID="LinkButton_Show_Report" runat="server" CssClass="report" Visible="False"><span></span>Show Report</asp:LinkButton>--%>
    ...
    <asp:LinkButton ID="LinkButton_print" runat="server" CssClass="print_btn" Style="width: 150px; background: #617b78;float: right;margin-left: 10px; color: white; padding: 0em 0em 2.1em 0em;border-radius: 3px; line-height: 2.2em;text-decoration: none; display:none;">Report With Header</asp:LinkButton>
     ...
     <asp:SqlDataSource ID="SqlDataSource_Consultant" runat="server" ConnectionString="<%$ ConnectionStrings:RadiologyConnectionString %>" SelectCommand="SELECT 0 AS EmpID, 'Select Consultant' AS EmpName UNION SELECT EmpID, ISNULL(EFName, '') + ' ' + ISNULL(EMName, '') + ' ' + ISNULL(ELName, '') AS EmpName FROM BasicDataInfo.dbo.Employee WHERE (Is_Consultant = 1) AND (DeptID = 27)">
     </asp:SqlDataSource>

                  <asp:SqlDataSource ID="SqlDataSource_Status" runat="server" ConnectionString="<%$ ConnectionStrings:RadiologyConnectionString %>"
                      ProviderName="<%$ ConnectionStrings:RadiologyConnectionString.ProviderName %>" SelectCommand="pt_select_status"
...
<asp:Label ID="Label1" runat="server" Text="Label" Visible="False" CssClass="err"></asp:Label><br />
        ...
        <asp:HiddenField ID="HiddenField3" runat="server" Value="0" /><asp:HiddenField ID="HiddenField_MS_TGID" runat="server" Value="0" /><asp:HiddenField ID="HiddenField_TSGID" runat="server" Value="0" />
                  ...
                  <asp:SqlDataSource ID="SqlDataSource_Update_Status" runat="server" ConnectionString="<%$ ConnectionStrings:RadiologyConnectionString %>"
                      ProviderName="<%$ ConnectionStrings:RadiologyConnectionString.ProviderName %>" UpdateCommand="Update_Patient_Test_Status" UpdateCommandType="StoredProcedure">
                      <UpdateParameters>
                          <asp:Parameter DefaultValue="6" Name="Status" />
                          <asp:SessionParameter Name="YearlyNo" SessionField="YearlyNo" />
                          <asp:SessionParameter Name="RegNo" SessionField="RegistrationNo" />
